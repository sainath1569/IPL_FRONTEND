...
const CreateAuction = () => {
  ...
  useEffect(() => {
    const fetchPlayerCount = async () => {
      try {
        ...
        const response = await fetch('https://ipl-server-mj6l.onrender.com/api/auction/getplayers');
    ...
    const fetchPlayers = async () => {
      try {
        ...
        const response = await fetch(
          `https://ipl-server-mj6l.onrender.com/api/auction/getplayers?${query.toString()}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://ipl-server-mj6l.onrender.com/api/auction/create', {
        method: 'POST',
        headers: {
